      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-x-0 top-16 bottom-0 z-40 overflow-y-auto bg-ivory-50 lg:hidden"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="flex h-full flex-col px-8 py-8">
              ...
              {navItems.map((item, i) => (
                ...
                >
                  <NavLink
                    ...
                    className="flex min-h-11 items-center border-b hairline py-4 font-display text-3xl font-medium text-ink-900"
                  >
                    {item.label}
                  </NavLink>
                </motion.div>
              ...
              <a
                href={`tel:${SITE.agent.phoneTel}`}
                onClick={() => setOpen(false)}
                className="mt-10 flex min-h-11 items-center justify-center gap-2 bg-ink-900 px-6 py-4 text-sm font-medium text-ivory-50"
              >
...
function LangToggle({
  ...
  return (
    <button
      ...
      className={`inline-flex h-11 min-w-11 items-center justify-center px-2 text-[11px] font-semibold tracking-[0.16em] transition ${
